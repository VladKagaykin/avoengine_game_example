...
fn main() {
    ...
    let mut floor = Draw_components{draw_type: "3d_object".to_string(), 
                                     draw_x: 256.0 /2.0 , 
                                     draw_y: 0.0 ,
                                     draw_z: 256.0/2.0, 
                                     draw_symbol: '█', 
                                     draw_vertices: vec![10.0,0.0,10.0,10.0,0.0,-10.0,-10.0,0.0,-10.0,-10.0,0.0,10.0],
                                     draw_RGBA_color: [255,255,255,255]};
    Draw_queue.lock().unwrap().push(square);
    Draw_queue.lock().unwrap().push(floor);
    drop(settings);
    Engine_setup();
    Setup_scene(256);
    ...
    // let mut hue: f32 = 0.0;
    // let size: f32 = 10.0;
    // let width = Engine_settings.lock().unwrap().window_width as f32;
    // let height = Engine_settings.lock().unwrap().window_height as f32;
    ...
    loop {
        ...
        
        {
            ...
            if camera.camera_pitch >=360.0{
                camera.camera_pitch= 0.0;
            }
        }
        ...
        {
            let mut camera = Camera.lock().unwrap();
            camera.camera_yaw += 90.0;
            if camera.camera_yaw >=360.0{
                camera.camera_yaw= 0.0;
            }
        }
